...
def plot_stats():
    if not os.path.exists("results/stats_mpi.txt"):
    ...
    with open("results/stats_mpi.txt", "r") as f:
        for line in f:
    ...
    plt.title('Зависимость времени умножения матриц от их размера (MPI)')
    plt.grid(True)
    plt.savefig("results/performance_plot_mpi.png")
    plt.show()
